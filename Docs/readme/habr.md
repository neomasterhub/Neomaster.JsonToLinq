...
Строй LINQ-выражения из JSON – <br>
интерфейс .NET-приложения, доступный всем!
...
**Если черный ящик поддерживает входные сигналы в форме каноничного JSON, то им могут пользоваться самые разные клиенты!**
...
1. GraphQL не использует каноничный JSON, требуя знания дополнительного синтаксиса. Это не только осложняет разработку, но и ухудшает интерфейс системы, повышая порог интеграции для клиентов – аналогично текстовым LINQ-выражениям и OData.
...
5. **Гибкость**
   ...
   Из коробки доступно добавление своих операторов, а сам функционал может послужить ядром для других проектов – например, для создания аналога [HotChocolate][hot-chocolate] с каноничным JSON и другими особенностями.
...
Таблица соответствия операторов методам обработки доступна через его свойство `Pairs`.
...
1. **Нет IntelliSense для написания JSON-фильтров**

   Без подсказок труднее контролировать правильность фильтров.

2. **Не поддерживаются поля агрегатов**
...
1. Движок GraphQL с каноничным JSON
